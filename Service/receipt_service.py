...
class ReceiptService:
    ...
    def _guess_category(self, name: str) -> int:
        ...
        # ── הסרת "בטעם X" / "טעם X" / "בטעמי X" / "בטעם של X" לפני ההתאמה ──
        # מונע מצב שבו "חטיף בטעם בצל" מקבל קטגוריית ירקות
        cleaned = re.sub(r'\bבטעם\s+\S+(?:\s+\S+)?(?:\s+\S+)?', '', name_lower)
        cleaned = re.sub(r'\bבטעמי\s+\S+(?:\s+\S+)?(?:\s+\S+)?', '', cleaned)
        cleaned = re.sub(r'\bבטעם של\s+\S+(?:\s+\S+)?(?:\s+\S+)?', '', cleaned)
        cleaned = re.sub(r'\bטעם\s+\S+(?:\s+\S+)?(?:\s+\S+)?', '', cleaned)
        cleaned = re.sub(r'\s+', ' ', cleaned).strip()
        if len(cleaned) < 2:
            cleaned = name_lower  # fallback — אל נמחק הכל

        # ── 37 = חטיפים וממתקים ──
        snacks = ["חטיף", "חטיפי", "ביסלי", "במבה", "צ'יטוס", "דוריטוס", "שוש",
                  "במבה", "אפרופו", "קליק", "פינגווין", "פיצוחים", "גרעינים",
                  "שוקולד", "ממתק", "סוכריות", "מרשמלו", "עוגיות", "ופל",
                  "בייגלה", "שלדג", "קרקרים", "פופקורן", "צ'יפס"]
        if any(w in cleaned for w in snacks):
            return 37

        # ── 32 = משקאות קלים ──
        drinks = ["קולה", "מיץ", "משקה", "סודה", "ספרינג", "פפסי",
                  "שוקו", "קקאו", "בירה", "יין", "שתייה", "שתיה",
                  "אנרגיה", "טרופית", "תפוזינה", "ספרייט", "פאנטה"]
        if any(w in cleaned for w in drinks):
            return 32

        # ── 27 = מאפים טריים ──
        bakery = ["עוגה", "עוגי", "בורקס", "קרואסון", "רוגלך", "סופגני", "דונאט",
                  "מאפה", "מאפין"]
        if any(w in cleaned for w in bakery):
            return 27

        # ── 31 = לחמים ותחליפי פחמימה ──
        bread = ["לחם", "חלה", "פיתה", "לחמני", "בייגל", "מצה", "בורגול", "קוסקוס"]
        if any(w in cleaned for w in bread):
            return 31

        # ── 35 = אורז ופסטות ──
        grains = ["פסטה", "ספגטי", "מקרוני", "אורז", "אטריות", "נודלס", "פתיתים"]
        if any(w in cleaned for w in grains):
            return 35

        # ── 30 = חלב וביצים ──
        dairy = ["חלב", "שמנת", "יוגורט", "גבינ", "לבן", "אשל", "קוטג'", "ביצ"]
        if any(w in cleaned for w in dairy):
            return 30

        # ── 29 = מוצרי חלב טריים (גבינות ספציפיות) ──
        cheese = ["צהובה", "גבינה", "בולגרית", "צפתית", "מוצרלה", "פרמזן", "גאודה", "עמק"]
        if any(w in cleaned for w in cheese):
            return 29

        # ── 34 = שימורים ורטבים (לפני ירקות — כי שימורי זיתים/תירס/אפונה ≠ טרי) ──
        canned = ["שימורי", "שימורים", "כבושים", "מוחמצים", "מוחמץ", "טונה", "רסק",
                  "קטשופ", "מיונז", "חרדל", "סילאן", "רוטב", "זיתים", "זיתי",
                  "קורנפלור", "מלפפון חמוץ", "סויה", "משומר"]
        if any(w in cleaned for w in canned):
            return 34

        # ── 39 = קפואים (לפני ירקות/בשר) ──
        frozen = ["קפוא", "קפואה", "קפואים", "קפואות"]
        if any(w in cleaned for w in frozen):
            return 39

        # ── 38 = בשר ודגים ──
        meat_fish = ["בשר", "עוף", "הודו", "סלמון", "קרפיון",
                     "אמנון", "בורי", "מושט", "נקניק", "נקניקיה", "פסטרמה",
                     "סלמי", "שווארמה", "קבב", "המבורגר", "שניצל",
                     "כנפיים", "שוקי", "חזה", "פרגית", "בקר",
                     "כבש", "עגל", "אנטריקוט", "צלעות", "סטייק"]
        if any(w in cleaned for w in meat_fish):
            return 38

        # ── 28 = פירות וירקות טריים ──
        # ⚠️  חובה לבדוק *אחרי* שימורים, קפואים, בשר, חטיפים
        fruits_veg = [
            # ירקות
            "עגבני", "מלפפון", "חציל", "בצל", "גזר", "כרוב", "חסה", "פלפל",
            "פטרוז", "קישוא", "דלעת", "סלק", "צנון", "שום", "אבוקדו",
            "בטטה", "תפו", "קולורבי", "ארטישוק", "אספרגוס", "שעועית ירוקה",
            # פירות
        ...
        if any(w in cleaned for w in fruits_veg):
            return 28

        # ── 36 = מוצרי ניקיון וטיפוח ──
        cleaning = ["נוזל רצפות", "סבון כלים", "מרכך כביסה", "אקונומיקה", "חומר ניקוי",
                    "ספוג", "מגבון", "שמפו", "משחת שיניים", "דאודורנט",
                    "מרכך שיער", "ג'ל", "קרם גוף", "תרחיץ", "נוזל כלים",
                    "מסיר", "אבקת כביסה", "טישו", "נייר טואלט", "חיתולים",
                    "מטליות", "שקית אשפה", "שקיות אשפה"]
        if any(w in cleaned for w in cleaning):
            return 36

        # ── 33 = מוצרי יסוד (כולל קטניות, תבלינים, שמן, סוכר, קמח) ──
        staples = ["סוכר", "מלח", "שמן", "קמח", "קטניות", "עדשים", "שעועית",
                   "חומוס", "תירס", "אפונה", "דבש", "ריבה", "טחינה",
                   "מרגרינה", "חמאה", "אורגנו", "פפריקה", "כמון",
                   "קינמון", "פלפל שחור", "כורכום", "זעתר", "תבלין",
                   "אבקת אפיה", "סודה לשתיה", "תמצית וניל", "שמרים",
                   "פירורי לחם", "ציר", "מרק", "אבקת מרק"]
        if any(w in cleaned for w in staples):
            return 33

        # ── 42 = חומרי ניקוי כבדים ──
        heavy_cleaning = ["אקונומיקה", "מסיר שומנים", "ספריי"]
        if any(w in cleaned for w in heavy_cleaning):
            return 42
